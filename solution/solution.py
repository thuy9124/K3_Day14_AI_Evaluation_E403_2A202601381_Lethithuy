...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for k, is_relevant in enumerate(relevant_flags, start=1):
            if is_relevant:
                relevant_seen += 1
                precision_at_k = relevant_seen / k
    ...
    def run_full_eval(
        ...
        if contexts:
            qa_pair.retrieved_contexts = contexts
        return EvalResult(
            qa_pair=qa_pair,
        ...
        try:
            ...
            if start != -1 and end != 0:
                ...
                if "scores" in data and "reasoning" in data:
                    return data
                return {"scores": data, "reasoning": "Parsed successfully."}
            raise ValueError
        except Exception:
            return {"scores": {k: 0.5 for k in rubric}, "reasoning": "Fallback"}
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        """
        if not results:
            return {"total": 0, "passed": 0, "pass_rate": 0.0, "avg_faithfulness": 0.0, "avg_relevance": 0.0, "avg_completeness": 0.0, "avg_context_recall": None, "avg_context_precision": None, "failure_types": {}}
        total = len(results)
        passed = sum(1 for r in results if r.passed)
        sum_f = sum(r.faithfulness for r in results)
        sum_r = sum(r.relevance for r in results)
        sum_c = sum(r.completeness for r in results)
        count_cr = sum(1 for r in results if r.context_recall is not None)
        sum_cr = sum(r.context_recall for r in results if r.context_recall is not None)
        count_cp = sum(1 for r in results if r.context_precision is not None)
        sum_cp = sum(r.context_precision for r in results if r.context_precision is not None)
        ftypes = {}
        for r in results:
            if r.failure_type:
                ftypes[r.failure_type] = ftypes.get(r.failure_type, 0) + 1
        return {
            "total": total,
            "passed": passed,
            "pass_rate": passed / total,
            "avg_faithfulness": sum_f / total,
            "avg_relevance": sum_r / total,
            "avg_completeness": sum_c / total,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for m, m_name in metrics_map.items():
            if base_rep.get(m, 0.0) - new_rep.get(m, 0.0) > 0.05:
                regs.append(m_name)
        return {
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        """
        scores = [(failure.faithfulness, "Context is missing or irrelevant — improve retrieval"), (failure.relevance, "Answer does not address the question — improve prompt clarity"), (failure.completeness, "Answer is missing key information — increase context window or improve generation")]
        min_score = min(s[0] for s in scores)
        if sum(1 for s in scores if s[0] == min_score) > 1 and min_score < 0.5:
            return "Multiple issues detected — review full pipeline"
        for score, cause in scores:
            if score == min_score:
                return cause
        return "Multiple issues detected — review full pipeline"
    ...
    def generate_improvement_suggestions(
        ...
        if cats.get("irrelevant", 0) > 0:
            suggestions.append("Refine system prompt to enforce answering exactly what is asked")
        ...
        if failures and len(suggestions) < 3:
            defaults = ["Improve embedding model for better retrieval", "Add guardrails for edge cases", "Conduct human review of failed queries"]
            for d in defaults:
...
def rerank_by_overlap(contexts: list[str], expected: str) -> list[str]:
    ...
    def overlap_score(chunk):
        ...
        if not chunk_tokens:
            return 0
        return len(expected_tokens & chunk_tokens)
